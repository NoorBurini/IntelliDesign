...
#from uploadimg import upload_photo
...
def receive_data():
    try:
        ...
        service_account_path = 'C:\\Users\\olabu\\Desktop\\imageupload\\imageupload\\service_account.json'
        if not os.path.exists(service_account_path):
            raise FileNotFoundError(f"Service account file not found: {service_account_path}")
        
        uploader = GoogleDriveUploader(service_account_file=service_account_path, parent_folder_id='1RXzDGO0ycDelT3t1eHbfgyOguWOhzuMk')
        image_url = uploader.upload_photo("C:\\Users\\olabu\\Desktop\\imageupload\\imageupload\\floorplan.png")
        print(f"Image URL: {image_url}")
        return jsonify({'message': 'ok', 'image_url': image_url}), 200
...
def draw_floorplan(solution, fitting_rooms_dict, total_area):
    #output_filename=r'C:\Users\user\OneDrive\Desktop\imageupload\floorplan.png'
